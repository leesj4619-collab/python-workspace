...
wc = WordCloud(
    ...
    scale=1,                     # 캔버스 비율
    relative-scaling=0.5         # 빈도 반영 강도(0~1)       
)
colormap= wordCloud에서 단어 색상을 지정하는 방법
-'viridis' = Matplotlib에서 제공하는 색상 맵
...
plt 내에 존재하는
interpolation = 보간법 = 알고 있는 데이터 지점들을 기반으로 그 사이의 값을 추정하여 데이터의 공백을 채우거나 곡선을 생성하는 데이터 처리 기법
...
def 기본문법():
    ...
    # 2. generate('문자데이터')
    # 문자열을 받아서 단어 빈도를 자동으로 계산하고 WordCloud를 만들어주는 기능
    # 문자열을 공백 기준으로 단어 분리
    # 각 단어 빈도 자동 계산
    # 빈도가 높은 단어일수록 크게 배치 -> wordCloud 이미지 생성
    # 반환
