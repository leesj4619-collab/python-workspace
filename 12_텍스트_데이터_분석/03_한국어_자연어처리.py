...
Mecab          매우 빠름   높음                별도 설치 필요
    ...
    -- 이후 한국어 버전인 Mecab-ko를 카카오 개발자들이 포팅
...
def okt기반():
    ...
    text='나는 파이썬으로 자연어 처리를 공부하고 있어'

    # 형태소 분리
    # .morphs(text) 형태소 분리
    # .morphs(text, stem=True, norm=True)
    # stem=True 어간 추출 (공부하고 -> 공부하다)  norm=True 정규화(ㅋㅋㅋㅋㅋㅋ -> ㅋㅋ)
    print(okt.morphs(text)) # ['나', '는', '파이썬', '으로', '자연어', '처리', '를', '공부', '하고', '있어']
...
인공지능 시대에 데이터 분석 능력은 매우 중요합니다.   
    ...
    plt.axis('off')
    plt.savefig('korean_word_cloud.png',dpi=150) # dpi Dots Per Inch 1인치 안에 점(픽셀)이 몇개 들어가는 회상도 단위
    # 높을 수록 선명하다!
    plt.show()
worcloud_okt_연결()
