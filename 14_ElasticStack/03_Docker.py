...
이미지(Image)
- 설치 설명서 + 재료 묶음
- springboot:3.4.5 이미지 = 스프링부트 3.4.5를 실행하는데 필요한 모든 것이 담긴 패키지
...
docker-compose.yml
- 여러 Docker 컨테이너를 한 번에 정의하고 실행하는 설정 파일
...
'''
도커는 언제 사용하는가?
= 내 컴퓨터에서 잘 작동해서 고객사에 코드 제품을 납품했는데 
고객사에서 안돼요. 를 없애기 위해서 사용

1. 개발 환경을 통일할 때 ( RAM이나 SDD와 같이 컴퓨터 부품에는 이상없다는 전제하)
 -- 팀원 A 는 Mac, 팀원 B는 Window, 서버는 Linux
  -> 도커 사용하면 각 컴퓨터 환경을 알 필요 없이 개발한 후 도커 파일 제공
    docker-compose.yaml을 제공하면 컴퓨터 os 환경에 관계없이 실행 가능
