'''
test code를 작성해본 경험이 있는자
MySQL Elasticsearch 검색 성능 벤치마크 도구

전체 구조
3개의 검색 케이스(CASE)를 MtSQL 과 ES에서 각각 실행해 속도 비교

CASES - 테스트할 검색 시나리오
케이스             내용
'삼성' 상품 검색    상품명에 '삼성' 포함 여부
설명 전문 검색       상품 설명에 "인체공학" 포함 여부
카테고리 + 가격 필터 카테고리 ='노트북' 이면서 가격 10~50만원
...
# time.time() 으로 쿼리 전후 시간을 제시 ms 단위로 계산
...
# size: 0 문서 내용은 안가져오고 건수만 죄회(빠름) 문서 내용 xxx 건수 ok
...
# m / e MySQL 시간 % ES = ES가 몇 배 빠른지 확인
