...
es.indices.refresh() # 인덱스 갱신 (=테이블 새로고침)
                        엘라스틱서치의 경우 관계형 데이터베이스에 존재하는 데이터를 기준으로 생성되는 것이기 때문
                        관계형 데이터베이스에서 수정 추가 삭제가 일어나면 그 상태를 그대로 반영

shard(샤드) 인덱스를 물리적으로 쪼개는 단위
인덱스(100만 건)
├─────── shard 0 (33만 건) → 서버 A
├─────── shard 1 (33만 건) → 서버 B
└─────── shard 2 (33만 건) → 서버 C
...
shard 0 (원본) -> 서버 A
shard 0 replicas -> 서버 B <- 서버 A가 죽어도 여기서 서빙 운영하는데 문제가 발생하지 않도록 설정

cursor DB에서 쿼리 결과를 순서대로 읽는 것
cursor.execute("select * from 테이블이름")
for 데이터한줄 in cursor: cursor로 가져온 "select * from 테이블" 결과를
...
for row in data:
    ex.index(index='테이블이름',body=row)

# 빠른 방식 (한 번에 처리)
helpers.bulk(es.action())

for 문 경우 : [요청] -> [응답] [요청] -> [응답] [요청] -> [응답]...반복
bulk  경우 : [요청요청요청요청요청요청요청요청] -> [응답응답응답응답응답응답응답]
...
request_timeout
응답 대기 최대 시간(초 단위)
return Elasticsearch(config.ES_HOST, request_timeout=30) # 연결 : 30초 안에 응답 없으면 에러
helpers.bulk(es, actions(), chunk_size=2000, request_timeout=60) # bulk : 60초 데이터가 많으므로 여유있게 연결
'''
...
def get_es():
    # ES_HOST    = "http://localhost:9200" docker에 9200으로 띄워놓은 엘라스틱 서치 연결
...
def create_index(es):
    ...
    es.indices.create(index=config.ES_INDEX, body={ # 어떤 데이터에 추가 수정 삭제가 일어났을지 모르니 초기화
        ...
        # text  = 검색 시 분석기 전용 (부분 검색 기능)
        # keyword = 있는 그대로 저장 (필터/집계/정렬에 사용)
        "mappings": {
            "properties": {
                "id":          {"type": "integer"}, # 정수형
...
def sync(es):
    ...
    # 성공에 대한 결과를 담는변수 , 실패에 대한 결과를 담는 변수 = helper.bulk(서버위치, 데이터, 한번에 http요청을 2000건씩 묶어서 보내겠다, 타임아웃 1분
