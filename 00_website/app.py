...
from = 가져온 flask 기능에서   import = 이 기능들을 사용하겠다.
    Flask = 웹 서버 만드는 기능
    render_template = templates에 존재하는 .html 파일 보여주는 기능

'''
from flask import Flask, render_template
# __name__ = 현재 app.py에 존재하는 파일 위치를 Flask에게 알려주는 것
# Flask(__name__) = 현재 app.py 파일을 기준으로 웹 서버를 생성하겠다.
# 이러한 모든 정보를 app 이라는 변수 공간에 담아두겠다.
app = Flask(__name__)
# Flask인 이유 Flask가 templates만 읽는다고 선언
