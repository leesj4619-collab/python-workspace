...
class JsonFormatter는 JSON으로 포맷하는 방법을 정의한 틀

def = 단순히 기능 하나
def get_logger는 로거(=기록) 하나 만들어줘 라는 함수(=기능)

둘을 class와 def로 나눈 이유는 
    JsonFormatter는 Python 로깅이 내부적으로 format() 메서드를 호출하는 규칙이 있다.
        반드시 class로 만들어서 내부에 format()을 작성해야한다.
    
    Python 자체적으로 log를 출력할 때 class 내에 format()으로 되어있는 기능을 가져와 라고 코드가 작성되어 있다.
    
Python logging : format() 이 작성된 문서(=class) 가져와
def format() -> format() 가져왔어요 -> Python : 기능 말고 기능이 들어있는 문서(=class) 가져오라고 error 발생
class JsonFormatter -> 이 클래스 안에 format()이 있아요 -> Python : ok 이대로 기록 진행할게요~
기록이라 하는 것은 회사나 개발 규모에 따라 방법이 매우 다양하기 때문에
파이썬에서 기록을 할 수 있는 어느 정도의 틀을 회사나 개발자에 요구하는 것이고
...
class JsonFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        범 죄 기록       -> criminal record
        로그 기록 1건    -> log record
        
        Python에서 logging에서의 의미
        개발자가 아래와 같은 코드 작성
...
# ai가 아래와 같은 방식을 알려줄 경우 -> logging.Logger 방식을 제외하고 알려줘! 요청
def get_logger(name:str) -> logging.Logger:
    ...
    # ('[%(levelname)s] %(name)s - %(message)s' = [INFO] routes - ES 검색 완료와 같이 표기))
