...
fake   = Faker("ko_KR")
log = get_logger('routes') # 2. 로그 변수 사용
...
def seed():
    ...
    for _ in range(count):
        try:
            ...
            db.session.rollback()
            # print(e) log보다 느리고 log파일로 기록할 수 없는 일회성 출력으로 회사에선 배포용으로 사용 안함
...
def stats():
    try:
        ...
        log.error("ES 카운트 조회 실패", extra={"x_error": str(e)},exc_info=True)  # TODO 2-1: 에러 로그
        es_count = 0
...
def get_orders():
    ...
    log.info("주문 목록 조회", extra={"x_page": page, "x_total": total})  # TODO 3: 페이지, 전체건수 기록
    return jsonify({
...
def search():
    ...
    log.info("ES 검색 완료", extra={"x_query": q, "x_hits": len(result)})  # TODO 5-2
    return jsonify(result)