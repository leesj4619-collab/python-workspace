...
#                                                        컬렉션 = 테이블 = 인덱스
#                                                               / 데이터베이스이름
클라이언트 = MongoClient('mongodb://admin:password@localhost:27018/')
'''
    ports:
      - "27017:27017"
    environment:
...
def 조회하기():
    ...
    # 조건 조회 컬렉션 = 데이터베이스['users']                         {} 데이터베이스['users']
    #user = 컬렉션.find_one({},{'name':'홍길동'})
    user = 컬렉션.find_one({'name':'홍길동'}) # select * from user
...
# crud를 import 했다해서 모든 기능을 수행하지 못하도록 설정
