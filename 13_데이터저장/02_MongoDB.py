'''
데이터를 JSON처럼 생긴 문서로 저장하는 데이터 베이스
...
def 몽고비디_파이썬코드_연결확인():
    # 연결 확인
    print('MongoDB 버전 : ', client.server_info()['version'])
...
# 모두 제거한 채 깨끗한 users 컬렉션에서 작업을 하고 싶다면 아래 명령어 주석 해지하고 실행
...
def insert_저장하기():
    ...
    print('한건저장결과확인 : ', 한건저장결과.inserted_id) # 저장된 id 조회
    # 여러 데이터 저장
    ...
    print('여러건저장결과 확인 : ', 여러건저장결과.inserted_ids)# 다수 데이터 저장된 id들 조회

def read_조회하기():
    # doc 단순한 변수 이름 document(문서) 줄임말 사용
    # result , data로 도 많이 사용
    ...
    # 1페이지 : LIMIT 10 OFFSET 0 -> 1번 ~ 10번 게시물
    # 2페이지 : LIMIT 10 OFFSET 10 -> 11번 ~ 20번 게시물
    # 3페이지 : LIMIT 10 OFFSET 20 -> 11번 ~ 30번 게시물

    # OFFSET = 여기서부터 시작할게 앞에건 무시하자!
