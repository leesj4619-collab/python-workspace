...
def 산점도확인():
    ...
    # 외우거나 익혀야할 필요 없음.
    for i in range(3): # setosa, versicolor, virginica 정답이 3개이므로 012까지 조회 3은 자동으로 하지 않는 방식
        plt.scatter( # 산점도 분포를 할 때

            # X= 데이터에서 y= 정답이 0==0일 때
            X[y == i, 0], # x축 : 꽃받침 길이 X[y == i, 0] X 에서 y == i 같은것만 조회하여 0번 열(꽃받침 길이만 갖고와)
            X[y == i, 1], # y축 : 꽃받침 너비 X[y == i, 0] y 에서 y == i 같은것만 조회하여 1번 열(꽃받침 너비만 갖고와)
            color=색깔[i],
            label=종류[i]
        )
    '''
