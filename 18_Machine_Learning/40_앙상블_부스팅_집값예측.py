...
def preprocess(train, test):
    ...
    # 판다스 최신 버전 cat_cols = train.select_dtypes(include=['object',str]).columns
    # 레거시 버전 cat_cols = train.select_dtypes(include='object').columns
...
def train_models(X_train, X_val, y_train, y_val):
    ...
    # 과제 2. XGBoost 모델 채우기
    xgb_model = xgb.XGBRegressor(n_estimators=100,random_state=42,eval_metric='logloss')
