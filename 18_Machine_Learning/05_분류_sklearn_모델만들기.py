...
# 3. 데이터의 정답만 y에 담아두기
y = iris.target
# 4. X,y에 저장된 데이터를 8:2 기준으로 train과 test에 각각 나눠서 저장하기
# 반드시 train에 오는 데이터는 학습용 데이터 - 정답으로 8~6에 준수하는 데이터와 정답 보유
# X_train = 데이터 80% y_train = 정답 80%

# 반드시 test에 오는 데이터는 학습이 잘 되었는지 확인용 데이터 - 2~4에 준수하는 데이터와 정답 보유
