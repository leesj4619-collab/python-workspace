...
# inplace = Ture == 방식이 pandas 최신 버전에서 사용하지 않는 방식
# train['Age'].fillna(train['Age'].mean(), inplace=True) 레거시 방식
...
# X(입력) y(정답)분리
X = train[columns]
y = train['Survived'] # 생존 유무 컬럼들 정답으로 사용해서 훈련시키겠다.
...
xgb_model = xgt.XGBClassifier(n_estimators=100,eval_metric='logloss', random_state=42)
...
submission.to_csv('csvs/titanic/submission',index=False)
