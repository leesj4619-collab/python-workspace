...
1. PCA(=Principal Component Analysis 주성분 분석)
    ...
    예) 100명의 학생 데이터가 존재
    키,몸무게,발사이즈,허리둘레,팔길이,.. 총 50개의 컬럼이 존재
...
4. LLE(= Locally Linear Embedding 지역 선형 임베딩)
    ...
    PCA 와 LLE 비교
    PCA는 전체를 한 번에 펼쳐서 확인
    LLE는 동네별로 쪼개서 펼침
    3D지구를 2D평면 지도로 만들 때
    PCA 방식 : 지구 전체를 한 번에 찍어서 납작하게 누름 왜곡 형상이 발생할 수 있다.
    LEE 방식 : 우선 가운데 선을 기준으로 조심조심 펼친다.
              남극 북극 쪽에는 데이터 훼손이 없는지 확인(크기)
              각 주변 국가들 관계를 유지하며서 펼친다.

5. NMF(비음수 행렬 분해)
    얼굴 사진이 있다. 얼굴 사진을 눈 코 입 같은 부분들의 합으로 분해하는 것
    모든 값이 양수여야 하며(= 이미지, 텍스트 적합) 어떤성분이 얼마나 기여했는지 파악하는게 중요

    피자(완성품) = 도우*0.4 + 소스*0.3 + 치즈*0.2 + 토핑*0.1
    위와 같이 각 부분들의 중요도를 변경하며 모델을 완성하는 것
    MMF가 하는 일
    완성된 피자 사진 1000장을 보고
    이 피자들은 공통적으로 도우 소스 치즈 토핑으로 이루어져 있구나 학습
    피자 1000장 = 각 피자의 재료 비율을 스캔 어떤 패턴으로 피자가 만들어졌는지 확인
        X       =    W       *         H
    피자1000장     각피자의         도우 치즈 소스
                 재료 비율  토핑 패턴과 같은 공통 패턴확인

    사진 1000장에서 어떤 피자는 치즈만 가득있고, 어떤 피자는 토핑만 있고 치즈 없으며
    어떤 피자는 소스가 안보이고와 같은 패턴들을 분석하며 재료 비율에 가중치를 설정
'''
...
# as는 import가 있어야지 쓸 수 있다!! from만 있어서는 사용 X
...
# print('헤드 확인 :', X.head()) head()는 csv 파일에서 사용
# print('헤드 확인 :', df.head()) 이런식으로 사용
...
scaler = StandardScaler()           # 스케일러라는 공간에 정규화 도구를 추가
X_scaled = scaler.fit_transform(X)  # 스케일러 안에 존재하는 정규화 도구를 이용해서
                                    # fit(평균/표준편차 계산) + transform(실제 변환작업)처리

colors = ['red', 'green', 'blue']   # 꽃 3종류가 어떻게 분포되어 있는지 색상으로 비교하기 위해
...
## 여기서 2차원으로 줄일 때 기준은? 길이 너비 중 2가지는 버려진 것
## 살아남은 2가지의 기준과 버려진 2가지의 기준
## PCA 코델 자체에서 학습을 할 때 중요한 컬럼 순서대로 줄을 세운 뒤, 내가 필요한 개수만큼
## 앞에서부터 자르는 것
## 데이터 평균을 낸 후, 모델 학습할 때 필요한 정도를 테스트하며 모델 완성
# 예) 1번 사진 꽃받침 길이 꽃받침 너비를 중요하게 생각해서 냅두고 모델 정답 -> 틀림
# 예) 2번 사진 꽃받침 너비 꽃받침 너비를 중요하게 생각해서 냅두고 모델 정답 -> 틀림
# 예) 3번 사진 꽃받침 길이 꽃잎 길이를 중요하게 생각해서 냅두고 모델 정답 -> 정답
...
print(f'분산 설명 비율 : {pca.explained_variance_ratio_}') # 각 주성분이 말하는 분산 비율
print(f'총 설명 분산 : {sum(pca.explained_variance_ratio_):.4f}') # 합계가 높을수록 손실 적다.
...
X_lda = lda.fit_transform(X_scaled, y) # 반드시 정답 데이터도 작업해야한다.
...
X_lle = lle.fit_transform(X_scaled) # 반드시 정답 데이터도 작업해야한다.
...
# 음수가 불가하여 StandardScaler 사용 X
# 0과 1 사이만 존재하는 MinMaxScalr 사용 O
...
print(f'판별 설명 비율 : {nmf.reconstruction_err_:.6f}') # 낮을수록 재구성 오류도 낮음
