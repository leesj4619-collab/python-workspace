...
import matplotlib.pyplot as plt # 그래프 그리는 도구 (시각화 해주는 도구)
...
# 1. K-Means
# =========================================================================
kmeans = KMeans(n_clusters=3,random_state=42) # 3개 그룹으로 나눠줘 AI에게 지시
# n_clusters=3 -> 3개의 그룹으로 나눠줘
# 몇 개의 구릅으로 나눌지는 개발자가 직접 지정
labels_km = kmeans.fit_predict(X) # AI가 데이터 보고 학습하고 바로 그룹번호 붙이기
# fit_predict = 학습하고 바로 그룹번호 반환
# 결과적으로 [0,1,2,0,1,2,0,1,2......] 나온다.

ax = axes[0] # 0번째 그래프
# 2차원 표(행,열) 데이터를 꺼내는 방법
# X 데이터는 아래와 같이 생겼다.
#       0번열     1번열
...
ax.scatter(
    kmeans.cluster_centers_[:,0], # 중심적 x좌표
    kmeans.cluster_centers_[:,1], # 중심적 y좌표
    c='black',
    marker='X',
    s=200,
    label='중심적'
...
# 2. DBSCAN
# 꼬인 데이에 강함
# make_moon 초승달 모양 2개 데이터 K-Means으로 나눌 수 없는 데이터
# =========================================================================
X_moon, _ = make_moons(n_samples=300, noise=0.1, random_state=42)
# 초승달 모양 데이터 300개 만들기(noise=0.1은 약간 흐트러진 느낌, 완벽한 초승달 말고
# 역간 삐뚤삐뚤한 초승달 그리기)
# n_samples 300개의 초승달 데이터
...
# 계층적 = Agglomerative = 아래에서 위로 합쳐가는 방식
