...
app = Flask(__name__) # 현재 app.py에서 실행하지 않을 경우 실행하는 py의 경로를 넣어줄 수 있다.
'''
# 1. 만들어놓은 모델 불러오기
...
def 예측하기():
    # 입력값을 html에서 가져오기
    # request.form['키이름'] = HTML from 태그에서 사용자가 입력한 값을 가져온다
    # html name = '키이름'으로 되어있는 태그의 값을 가져온다.
    # input의 경우 기본으로 전달받는 데이터는 모두다 string 문자열 형태로
    ...
    # if 문에서 숫자 index가 0일 경우 False로 처리되어 데이터가 보이지 않는 이슈 발생
    # 개발자는 0,1,2처럼 나오는 데이터를 소비자가 알 수 있는 형태로 변환하여
    # 소비자에게 보여줄 의무가 있다.
    # 소비자가 불편한 사이트는 개발자가 개발할 때 편했던 사이트
    # 개발자가 불편했던 사이트는 소비자가 편한 사이트
