...
- Naive = 단순한 / 세상 물정 모르고 당순하게 생각한다 가정한다 의미
...
예) 스팸 메일
이메일에 무료 당첨이라는 단어가 있으면 스팸인가요?!
스팸일확률 = (스팸 메일 중 "무료"가 포함된 비율) X (스팸 매일 중 "당첨"이 포함된 비율) X (전체 스팸 비율)

각 단어가 서로 독립적이고 가정(navie)하고 곱해서 계산하는게 포인트

실제 사람들이 작성하는 문서 문장에는 단어들이 서로 연관되어 있지만, 나이브 베이즈는 '모두 독립적이다'
라고 단순하게 가정한다.
틀린 가정일 수 있지만 실제로 꽤 작동하는 알고리즘
'''
import pandas as pd
from scipy.ndimage import vectorized_filter
from sklearn.externals.array_api_compat.torch import result_type
# 나이브베이즈 기초 코드
from sklearn.naive_bayes import MultinomialNB               # 확률을 학습하는 기능
...
def 기초코드문법():
    ...
    train_test_split
    - 데이터가 100개 이상일 때 사용
    - 데이터가 100개 이하면 사실 너무 데이터가 적어서 의미가 크게 있진 않아.
    - SNS Spam(5572개) 필수로 사용해야한다.
    '''
...
def csv_기초코드문법():
    ...
    MultinomialNB를 사용하려면 반드시 CountVectorizer() 기능을 이용해서 글자 -> 숫자 변환하는 작업을 해준 다음 모델을 쓸 수 있다.
    '''
    ...
    '''
    X_test_숫자로변경 = vectorizer.transform(X_test)       # transform = 훈련이 제대로 됐는지 확인된 데이터만 선택
...
'''
1. 엘라스틱서치에서 형태소로 분리해서 단어 찾기
   나이브 베이즈 이 두가지 차이점이 무엇인가.
    
        WordCloud       나이브 베이즈
목적    보여주기(시각화)    판단하기(분류)
결과물     이미지         예측값(스팸/정상)
질문    어떤 단어가 많은가? 이게 스팸인지 아닌지 판단

WordCloud   = 텍스트 안에 뭐가 있는지 눈으로 보자고
나이브 베이즈  = 이 텍스트가 어떤 종류인지 결정하자

언제 무엇을 사용하는가?
리뷰 1만 개 있다
어떤 불만이 많은지 파악하고 싶다 -> WordCloud
불만 리뷰인지 자동 분류하고 싶다 -> 나이브 베이즈

데이터 처음 탐색할 때             WordCloud
보고서/발표 자료 만들 때          WordCloud
스팸  자동  필터링               나이브베이즈
감정 분석(긍정/부정 자동 분류)     나이브베이즈
뉴스 카테고리 자동 분류           나이브베이즈

실제 실무에서는 둘이 같이 혼용해서 사용
1단계 WordCloud 이용해서 데이터 탐색
     '아.. 스팸에 무료/당첨 단어가 많네'
     
2단계 나이브 베이즈로 자동 분류기 만들기
    '무료/당첨'이 있으면 자동으로 스팸처리 하겠다

WordCloud   = 탐색 도구
나이브 베이즈  = 자동화 도구

2. KNN 선형 회귀, 로지스틱, 나이브베이즈 정리

                KNN         선형회귀        로지스틱        나이브베이즈
목적           분류/예측       숫자예측         분류            분류
결과물         클래스/숫자      연속된숫자       확률(0~1)       클래스
언제사용하는가   가까운 이웃      결과로 직선그리기 확률로 나누기     조건부 확률
데이터량 확인    많을수록 좋다.   정도껏          정도껏          적어도 된다.

코드를 어떻게 쓰는가
모델에는 논문 / 깃허브 모델을 사용하는 방법 코드 올려놓는다.
방법 코드를 참고해서 초기에 모델을 이용해보고 각 회사와 개발자가 계산한 수학 값으로 모델에 전처리 계산을 다르게 하여

데이터 작업 / 컬럼 작업 -> 전처리 
전처리 제일 어려움
-> 앞서 작성한 train_test_split 언제 fit_transform transform 사용하고
어떤 컬럼을 선택해서 사용하고 와 같이 언제 무엇을 전처리 해야하는가 에 대한 정의를 내린 다는 것 굉장히 어렵다.
회사가 원하는 결과에 도달하기 위하여 굉장히 많은 숫자값을 테스트하여 정확도를 높여야한다.
'''