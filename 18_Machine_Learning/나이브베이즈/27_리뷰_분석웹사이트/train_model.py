...
for 이름,ID in 앱목록.items():
    # result만 필요한데 reviews라는 기능이 2가지를 반환하여
    # 오른쪽은 사용하지 않고 자리만 맞추겠다 _ 사용
    '''
    reviews(
    ID,                 어플의 고유 ID = com.naver.shopping도메인 반대
    lang='ko',          리뷰언어 ko = 한국어, en = 영어
    country='kr',       국가 kr = 한국 us = 미국
    sort=Sort.NEWEST,   정렬방식 NEWEST(= 최신순) RATING(=평점순) HELPFULNESS(=도움된 순)
    count=2000          수집할 리뷰 개수
    )
    
    result , _
    리뷰      next_token, 다음페이지 이동용
    목록      다음페이지를 사용할 일이 없기 떄문에 _ 버리는 것
...
# 어플에 작성된 리뷰 데이터들도 필요하기 때문에 백터라이즈까지 함께 저장
# =============================
