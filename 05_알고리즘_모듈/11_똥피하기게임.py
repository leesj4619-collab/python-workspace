...
플레이어_y = 530
플레이어_크기 = 35
속도 = 6      # TODO 1: 플레이어 이동 속도

# 똥 설정
똥목록 = []
똥속도 = 4     # TODO 2: 똥 떨어지는 속도
타이머 = 0
...
while 실행중:
    ...
    for 이벤트 in pygame.event.get():
        ...
        if 이벤트.type == pygame.KEYDOWN:
            if 게임오버 and 이벤트.key == pygame.K_r:
                똥목록.clear()
                플레이어_x = 400
                점수, 똥속도, 게임오버 = 0, 4, False   # TODO 3: 재시작 초기 똥속도 (TODO 2랑 같은 숫자)

    if not 게임오버:

        키 = pygame.key.get_pressed()
        if 키[pygame.K_LEFT]:  플레이어_x -= 속도         # TODO 4: 왼쪽 이동 (속도 변수)
        if 키[pygame.K_RIGHT]: 플레이어_x += 속도         # TODO 5: 오른쪽 이동 (속도 변수)
        플레이어_x = max(플레이어_크기, min(800 - 35, 플레이어_x))  # TODO 6: 화면 가로 크기
    ...
    for 똥 in 똥목록:
        ...
        점수 += 1
        if 점수 % 200 == 0: 똥속도 += 1             # TODO 12: 난이도 증가
