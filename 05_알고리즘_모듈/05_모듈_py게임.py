...
pygame
└ pygame(원조) / pygame-ce(활발하게 개발 중)
'''
# import tkinter as tk -> 파이썬 개발자가 만들었음 만든사람들의 룰이 있음
import pygame # pete shinners -> C 언어를 사용하는 개발자인데 파이썬에서 게임 개발 도구가 부족하다 판단해서 개발
# 코드 공개하고 추가보수 기여를 2006 이후로 거의 안했으나 파이썬 pygame 사용하는 커뮤니티 유저들이 개발 유지
...
while 실행중:
    ...
    # 키보드 입력으로 공 이동
    키 = pygame.key.get_pressed()        # 키 안에 모든 키의 상태변수가 들어있는 상태
    if 키[pygame.K_LEFT]: 공_x -= 속도    # 공x 가로 위치를 x가 줄면 왼쪽 이동
    if 키[pygame.K_RIGHT]: 공_x += 속도   # 공x가 늘면 속도만큼 오른쪽으로 이동
    if 키[pygame.K_UP]: 공_y -= 속도      # 공y가 줄면 위쪽으로 이동
    if 키[pygame.K_DOWN]: 공_y += 속도    # 공y가 늘면 아래쪽으로 이동
