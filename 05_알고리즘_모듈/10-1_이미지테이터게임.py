...
배경 = pygame.transform.scale(pygame.image.load("images/running/img.png").convert(), (800, 600))
땅이미지 = pygame.transform.scale(pygame.image.load("images/running/img_1.png").convert_alpha(), (800, 120))
플레이어이미지 = pygame.transform.scale(pygame.image.load("images/running/img_2.png").convert_alpha(), (70, 70))
장애물이미지 = pygame.transform.scale(pygame.image.load("images/running/img_3.png").convert_alpha(), (60, 60))
...
while 실행중:
    ...
    if not 게임오버:
        ...
        # ------------------------------------------------
        # TODO 1: 스페이스바를 꾸욱 누르는 동안 높이 점프하기
        # 현재 코드는 KEYDOWN 이라 한 번만 눌려서 살짝만 점프함
        # key.get_pressed() 로 누르고 있는 동안 계속 위로 올라가게 만들어보기
        #
        # 힌트:
        #   if 키[pygame.K_SPACE] and ???:   # 땅위일 때만? 공중일 때도?
        #       속도_y = ???                 # 얼마나 올라갈지
        #
        # 현재 점프력: 속도_y = -16
        # 꾸욱 누를수록 더 높이 → 속도_y를 어떻게 바꾸면 될까?
        # ------------------------------------------------
        ...
        # ------------------------------------------------
        # TODO 2: 좌우 이동 (← → 방향키)
        # 플레이어_x 를 키 입력으로 움직여보기
        # 화면 밖으로 나가지 않게 막는 것도 도전!
        #
        # 힌트:
        #   if 키[pygame.K_LEFT]:  플레이어_x -= ???
        #   if 키[pygame.K_RIGHT]: 플레이어_x += ???
        #
        #   화면 밖 막기:
        #   플레이어_x = max(???, min(???, 플레이어_x))
        # ------------------------------------------------
