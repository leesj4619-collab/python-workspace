...
def 폴더작업():
    # 폴더 생성
    os.makedirs('새폴더') # 폴더만들기 동일한 명치의 폴더가 이미 존재하면 에러 발생
    ...
    폴더생성 = 'seaborn_save'
    os.makedirs(폴더생성,exist_ok=True)
...
def 파일작업():
    ...
    # 파일이름만 갖고오기
    os.path.basename('폴더1번/폴더2번/폴더3/파일이름.확장자') # 파일이름.확장자

    # 파일이름과 확장자 분리
    os.path.splitext('파일이름.확장자') # ('파일이름', '.csv')

    os.path.join('폴더1/폴더2/폴더3','파일이름.확장자')