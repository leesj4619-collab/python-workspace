...
def say_hello():
    print("안녕!!!") # 출력만 하고 끝이다.
###   return이 있는 함수(=기능)
결과1번 = say_hello() # 실행은 가능!
print(결과1번) # None 나는 결과 1번에서 return으로 받은 데이터가 없다!
###   return이 있는 함수(=기능)
def 더하기기능(a,b):
    return a+b # 값을 반환해줌
결과2번 = 더하기기능(3,5)
...
greet('철수')                             # 기본 msg 사용
greet('영희','반갑습니다')        #
#greet('영희','반갑습니다', 'ㅎㅎ') # 매개변슈 데이터로 존재하지 않는 데이터는 작성 불가
...
## 3. 가변 인수
# *args : 개수 제한없이 받기 (튜플로 들어옴) 튜플은 받아온 값을 수정 불가
# 데이터를 받아올 것인데 개수 제한을 하고싶지 않을 때 사용
def 총합(*args):
    return sum(args)
print(총합(1,2,3,4,5))

# **kwargs : 키워드 인수 여러 개 받기 (딕셔너리 들어옴) { "키이름", "데이터" } Map형태
def 정보(**kwargs):
