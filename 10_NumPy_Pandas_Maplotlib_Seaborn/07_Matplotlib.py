...
그래프의 모든것을 직접 제어할 수 있다.
'''
# matplotlib 라이브러리에서 특정 기능 pyplot을 가지고와서 그래프를 그릴 것인데
...
# - 만 깨짐 설정 방지하는 이유는 matplotlib 기본 설정이 유니코드 마이너스 기호를 사용하도록 세팅되어있고
# 한글 폰트로 바꾸면 그 폰트에 해당하는 특수문자가 없어서 ㅁ 또는- 대신 깨진 문자가 보임
...
# 보여진 그래프는 x창을 누르는 순간 메모리에서 사라지게된다.
