...
# 참고로 한국의 경우 Nominatim을 이용하는 것보다 Kakao에서
...
def 기본문법():
    ...
    # 위도 경도가 없는 주소는 에러 발생 try 예외처리 혹은 데이터 전처리를 해야한다.

def csv_주소_변환():
    df = pd.read_csv('데이터실습파일/전국도서관표준데이터.csv',encoding='cp949')
    #                               데이터가 없는 행 데이터 정보들만 따로 복사해서 사용하겠다.
    df_위도가없는데이터 = df[df['위도'].isna()].copy() # 주소는 존재하나, 위도 경도가 csv 데이터로 없을 경유
...
def csv_주소_변환_데이터테스트():
    df = pd.read_csv('데이터실습파일/전국도서관표준데이터.csv',encoding='cp949')
    #                               데이터가 없는 행 데이터 정보들만 따로 복사해서 사용하겠다.
    df_위도가없는데이터 = df[df['위도'].isna()].copy()
    print('df_위도가없는데이터 개수 확인 : ', len(df_위도가없는데이터))
    df_위도가없는데이터 = df[df['위도'].isna()].head(1) # 상위 10개의 데이터만 존재
    print('df_위도가없는데이터 개수 확인 : ', df_위도가없는데이터)
    # 위도 경도가 없는 행만 갖고온 후 데이터 채우기
    # dropna = NaN 있는 행 버리기
    # isna = 없는 행만 추출하기
