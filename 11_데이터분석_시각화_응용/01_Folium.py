...
** 클러스터링 : 마커가 너무 많을 때 뭉쳐서 보여주는 것
            예를 들어 마커 2799개 -> 그냥 찍으면 지도가 점으로 가득 참
...
def folium기본():
    ...
    ##### 마우스를 살~짝 올리면 보여질 텍스트 툴팁 추가 folium에서 제공하는 아이콘 세팅
    ###### .add_to(m) 모든 세팅을 끝낸 결과를 m 공간에 추가하겠다.
    folium.Marker(
        location=[37.5665, 126.9780],
        popup='서울시청', # 클릭하면 뜨는 팝업
        tooltip='여기를 클릭', # 마우스를 올려두면 뜨는 텍스트 = hover
...
def folium_excel_위도경도():
    # 인텔리제이에서 현재폴더기준 유사명칭 파일들 호출이 read_cvs는 잘 되지만
    # read_excel에선 잘 되지 않는다.
    # header = None 첫 줄을 컬럼명으로 사용하지 말 것
    # skiprows = 4 위에서 4줄은 건너뛰기
    df = pd.read_excel('공공자전거_대여소_정보(25.12월_기준).xlsx',
    ...
    df.columns = ["대여소번호",'대여소명','자치구','상세주소',
                  '위도','경도','설치시기','LCD거치대','QR거치대',
                  '운영방식'
                  ]
    ...
    # 마커 찍기를 하기 전에 위도 경도가 비어있는 행을 버리거나 0으로 만들거나
    # 기획자와 회사가 결정한 사항에 따라서 처리
    # 위와 같은 사항을 데이터 전처리 -> 데이터로 결과를 만들기 전 전부다 처리
...
def folium_excel_클러스터링():
    ...
    for _, row in df.iterrows():
        folium.Marker(
            location=[row['위도'],row['경도']]
        ).add_to(cluster) # m이 아닌 클러스터에 추가
        # 위도 경도를 마커가 너무 많으면 동그라미 표기해주는 곳에 세팅할 거고
        # 동그라미 표기법은 folium에서 제공하는 지도 형태를 따를 것.
    m.save('따릉이지도.html')
