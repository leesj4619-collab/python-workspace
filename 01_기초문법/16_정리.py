...
# 변수 공간에 담아서 보관한다.
# input의 경우 소비자가 어떤 데이터를 작성할지 모르기 때문에 무조건 String으로 데이터를 가져온다.
...
def input_int_ry기능():
    ...
    while True:
        try:
            ...
            print(f"{소비자입력데이터}님의 나이는 {소비자숫자데이터}살 입니다.")
            break
        except ValueError:
...
'''
for문은 종류가 괸장히 많다.
for 하나씩_데이터를_꺼내와서_사용할_변수이름 in 순회할_것들 : (ex: for i in range:)
    순회하면서 진행할 코드 작업들 작성
    
숫자 순회 range
for 숫자데이터 in range(5):  0에서 부터 4까지 순회
    print(숫자데이터)

for 숫자데이터 in range(1, 6):  1에서 부터 5까지 순회
    print(숫자데이터)
    
for 숫자데이터 in range(0, 10, 2):  0에서 부터 9까지 2씩 증가하여 순회
    print(숫자데이터)    

for 숫자데이터 in range(5, 0, -1):  5에서 부터 -1씩 감소하여 0이 되기 직적인 1까지만 순회
    print(숫자데이터)
...
번호 붙이며 순회
목록들 = ["일",2,"삼",4,True]
for 번호매김,목록데이터 in enumerate(목록데이터,start=1):  목록에 있는 데이터에 1부터 번호를 매겨 작업
    print(f"{번호매김}번: {목록데이터}")                  start를 생략하면 0번부터 번호매김을 진행
...
def forelse():
    for i in range(5):
        ...
        print("반복 완료") # for안에서 break 없이 끝나면 실행
#forelse()

'''
def ifelse():
    if 조건1:
        # 조건 1이 True일 때 수행할 구문
    elif 조건2:
        # 조건1이 False이고 조건2가 True일 때 수행할 구문
    elif 조건3:
        # 조건1&2이 False이고 조건3가 True일 때 수행할 구문
    else :
        # 위 조건이 모두 False일 때 수행할 구문
'''


'''
ValueError: invalid literal for int() with base 10: '스물살'
