...
TensorFlow
- 구글이 만든 딥러닝 프레임워크
- TensorFlow는 행렬 미분, 역전파, GPU 메모리 관리를 모두 작성해야하는 번거로움 해소 (처음 도움을 받을 때만 사용)
...
구글에서 인정하는 탠서플로우 자격증은 그렇게까지 따는 추세는 지났다.
자격증 (=AICE - 국가에서 인정되는 단계 자격증 취득하는 것이 낫다.)
...
'''
# TensorFlow 2.11 버전부터 windows 환경에서는 GPU를 공식 지원하지 않는다.
# 구글이 정책을 바꿨으며, CUDA를 설치했어도 GPU 인식이 안된다.
# 텐서플로우 2.21 버전이기 때문이다.
WARNING:tensorflow:TensorFlow GPU support is not available on native Windows for TensorFlow
 >= 2.11. Even if CUDA/cuDNN are installed, GPU will not be used.
...
Microsoft가 만든 WindowsGPU 지원 플러그인이다.
pip install tensorflow-cpu
pip install tensorflow-directml-plugin
설치필요

딥러닝을 개발하는 개발자들이 원도우 컴퓨터가 아니라 ubuntu나 맥북과 같은 컴퓨터
aws 와 같은 클라우드에서 딥러닝개발하는 것을 구글에서 확인

구글 입장에서는 window gpu 지원을 유지하는게 유지보수 비용 대비 사용자가 적다.
구글자체에서 클라우드 판매
...
- 당시 컴퓨터가 만들어진 목적은 계산
  급여 계싼, 회계 장부, 미사일 탄도 계산과 같은 반복적인 작업의 특징을 파악하여 기계로 작업하였다.
  CPU 컴퓨터 설계 철학 : 하나의 복잡한 일을 빠르게
            워드 키고, 파일 저장하고, 인터넷 연결하는 작업들은 전부 순서가 있고 복잡
            CPU는 이러한 일들을 잘하기 위하여 생성된 부품

GPU
- 1990년대 들어서 PC게임 시장이 커졌다.
- 게임 화면을 그리려면 모니터의 픽셀을 전부 계산
  예) 모니터 사이즈 1920 X 1080 사이즈면 하나의 화면을 그리기 위해서 필요한 픽셀
  207만개 정도가 필요 초당 60프레임 1초에 1억 2천만 개의 픽셀 생상을 계산해야한다.
  
  픽셀 계산의 특징이 있다. 각 픽셀은 서로 독립적이다.
  CPU로 계산을 하게 되면 순차작업으로 하나씩 계산하게 되어 너무 느리다.
  
  NVIDIA 1999년 GPU 처음 생성
