...
사람으로 비유:
책 한 권을 읽고 -> 한 줄 요약 -> 그 요약으로 다시 책 내용 다시 설명

'''
import tensorflow as tf     # 아래 from를 코드를 작성하지 않아도 사용할 수 있으나
...
# 이걸 작성하지 않았다면 tf.keras.layers.Dense
# 이걸 작성하지 않았다면 tf.keras.layers.Input
# 매번 앞에다가 텐서플로우.케라스.레이어즈.에서 가져온 Input 기능이다 와 같이 표기
from tensorflow.keras.models import Model, Sequential
# 이걸 작성하지 않았다면 tf.keras.models.Model
# 이걸 작성하지 않았다면 tf.keras.models.Sequential
...
# from tensorflow.keras.datasets import mnist을 작성하면
# 아래와 같이 단축하여 작성할 수 있다.
...
불량품 사진은 거의 없다.
정답 y를 만들 수가 없다.
나는 모델 못만드니 퇴사해야하나..? 가 아니라
오토인코더를 이용해서 정상만 학습 -> 정상과 다른 것 탐지하기 위하여 존재
...
로봇뇌 = Sequential([
    Input(shape=(784,)),
    # Dense 안에 8 16 32 64 128 256 512 와 같은 8자리 숫자를 사용할 의무 없음
    # 업계 관습 상 GPU 메모리 구조가 32와 같은 8 표기법 구조에 최적화 되어 있어
    # 계산이 빠르기 때문에 사용 200, 100, 300 쓰고 싶은 숫자 작성해도 된다.
    Dense(256, activation='relu'),  # 인코더 : 압축
    # 중요한 데이터를 제외하고 억지로 256개에 784개 데이터를 구겨 넣는다.
    # 정말 중요한 256개
    Dense(128, activation="relu"),  # 정말 중요한 128개만 놔둔다.
    Dense(64, activation="relu"),   # 잠재벡터(병목) 최종적으로 중요한 64개만 냅둔다.
...
로봇뇌.fit(
    ...
    validation_split=0.1,   # 훈련데이터 10%는 검증용으로 빼둠 sklearn = 머신러닝이 아니기 때문에 train_test_split(test_size=0.8)을 사용하지 않은 것일 뿐!
                            # 텐서플로우 = 딥러닝에서 사이킷런 = 머신러닝에서 사용한
                            # 데이터 분리 작어 기능을 해도 되지만 보통은
                            # 딥러닝에서는 딥러닝 회사 분류 기능을 사용하려 하고,
                            # 머신러닝에서는 머신러닝 회사 분류 기능을 사용해주려 한다.
    verbose=1               # 학습 진행상황 출력0=안보인 1=모두 보임 2= 결과만 보임
)

# 5. 모델 저장하기

# AI의 경우 1~5번 사이 순서가 매우 중요
# 6. 시각화
