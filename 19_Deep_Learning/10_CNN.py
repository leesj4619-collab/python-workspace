...
각 숫자 = 픽셀밝기(0=검정, 255=흰색) # ffffff = 255 255 255 와 같은 255로 이루어진 흰색
...
필터가 찾은 것들 :
    ...
    마지막 레이어  : 이건 고양이다, 이건 자동차다 와 같은 결론
    Conv2D(64,(3,3))
...
로봇뇌.compile(
    ...
    # 학승방법은 난다 긴다하는 수학가이자 개발자들이 작성한 대표적인 공식들 중 하나 보통 사용
    loss='sparse_categorical_crossentropy', # 틀린 정도 측정 3개 이상은 옆에 있는 loss 많이 사용
