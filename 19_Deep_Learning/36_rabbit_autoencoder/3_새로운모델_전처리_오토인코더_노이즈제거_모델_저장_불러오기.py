...
print(f"최종 데이터 형태: {토끼_3D.shape}")
# 예상 출력: (이미지수, 12288)

# 훈련 80% / 테스트 20% 분리
훈련, 테스트 = train_test_split(토끼_3D, test_size=0.2, random_state=42)
...
로봇뇌 = Sequential([
    # ★ 핵심: 입력을 3차원 (64, 64, 3)으로 받습니다.
    Input(shape=(이미지크기, 이미지크기, 3)),
    ...
    MaxPooling2D((2, 2), padding="same"), # 최종 압축 상태
    ...
    # 출력층: 채널 수를 원래대로 3(RGB)으로 맞추고 0~1 사이 값 출력을 위해 sigmoid 사용
...
# 오토인코더 학습 (훈련 데이터의 차원이 3D이므로 그대로 넣어줍니다)
로봇뇌.fit(
    훈련, 훈련,
    epochs=50,
    batch_size=32,
...
for i in range(5):
    # ★ 중요: .reshape() 제거
    축[0, i].imshow(테스트[i])
    축[1, i].imshow(테스트노이즈[i])
    축[2, i].imshow(복원이미지[i])
    for j in range(3):
        축[j, i].axis("off")
...
CNN_로봇뇌.save("autoencoder.keras")            # (20) "autoencoder.keras"
