# warning 메세지가 보기 싫을 때 끄는 방법 (추천하지 않음)
import os
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0' # ONDNN만 안하겠다.
os.environ['TF_CPP_MIN_LEVEL'] = '3' # 0 = 전부 출력, 1=INFO제거 2= WARNING제거 3= 전부출력제거
...
귀로빈섬(=파이썬창시자)가 만든 random과 착각해선 안된다.
둘은 다른 것이다.
...
============================= 위 세가지 많이 사용 =============================
 ...
 - .truncated_normal() = normal과 비슷한데 크기가 너무 크거나 작은 값은 잘라내겠다.
                                                 안정적으로 초기화 진행할 때
...
기본층
 ...
 이미지용
 - .Conv2D = 이미지에 특징을 찾아내는 층으로 사진을 인식할 때 사용
 -      1층 Conv2D 에서 선 점 같은 단순한 특징을 찾고
 -      2층 Conv2D 에서 눈 코 같은 복잡한 특징을 찾고
 -      3층 Conv2D 에서 얼굴 전체 같은 고차원 특징을 찾는다. 와 같은 뇌를 개발자가 작성
 - .MaxPooling2D = 이미지 크기를 줄여주는 층 / 학습할 때 이미지가 너무 크면 시간이 오래걸리므로
                   이미지 사이즈를 임의적으로 줄여서 사용
                   Conv2D와 같이 사용
                   보통 Conv2D를 이용한 다음에 Conv2D에 존재하는 데이터를 바탕으로
                   중요해보이는 데이터만 남겨두고 나머지 필요없는 데이터를 버릴 때 사용
                   Conv2D -> MaxPooling2D 이런식으로 많이 사용
                   MaxPooling2D -> Conv2D 이렇게는 사용하지 않는다. (특수하지 않는 한)
                   
 - .Flatten    = 2D이미지를 1D 일자로 펴주는 층
                 Conv2D 로 (표형태)로 나온 결과를 Dense로 작업해야할 때 중간에 사용
                 Dense 같은 경우 표가 아닌 목록 형태로 되어 있는 데이터만 받아서 사용할 수 있다.
                 만약에 Conv2D에서 Dense로 가야할 일이 있을 때 중간에
                 Conv2D -> Flatten -> Dense 넣어 왼쪽과 같이 사용
 텍스트/순서용
 - .LSTM       = 앞 뒤 순서를 기억하는 층. 문자, 주가 예측할 때 사용
                 문장의 순서를 기억할 때 사영
                 Dense의 경우 나는 / 밥을 / 먹었다 에서 순서를 모른다.
                 LSTM 의 경우 나는 -> 밥을 -> 먹었다 흐름 순서를 기억한다.
                 글자에서는 Dense 보다 LSTM을 사용한다.
...
mse
epochs
- 같은 데이터를 몇 번해서 반복하여 공부할 지 기입
- 사람도 한 번봐서 모르는 것처럼 같은 데이터를 여러 번 봐야 외우듯이 AI도 여러번
  보게끔 하여 공부시킨다.
- 에포크가 클수록 메모리나 gpu 사용량도 커진다. 컴퓨터 사양에 따라 적절히 숫자도 커진다.
verbose
...
I0000 00:00:1780281488.987025    7680 port.cc:153] 
